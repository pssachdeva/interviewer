...
def test_build_cluster_label_rows_uses_centroid_nearest_examples(tmp_path) -> None:
    ...
    assert "rationale=The speaker avoids AI rewriting to preserve voice." in first_cluster["representative_snippets"]
...
def test_build_cluster_label_rows_can_use_evidence(tmp_path) -> None:
    ...
    points.to_csv(mechanism_dir / "clustered_rationales.csv", index=False)
    np.save(mechanism_dir / "embeddings.npy", np.asarray([[0.0, 0.0], [0.1, 0.0]], dtype=np.float32))
    ...
    assert rows.loc[0, "snippet_source_label"] == "Quoted evidence excerpts selected for the mechanism"
